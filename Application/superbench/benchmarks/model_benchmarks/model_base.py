...
from Application.superbench.common.utils import logger, stdout_logger
from Application.superbench.benchmarks import Precision, ModelAction, DistributedImpl, DistributedBackend, BenchmarkType, ReturnCode
from Application.superbench.benchmarks.base import Benchmark
from Application.superbench.benchmarks.context import Enum

from Application.superbench.benchmarks.power_monitor import PowerMonitor
...
class ModelBenchmark(Benchmark):
    ...
    def __train(self, precision):
        ...
        result_string = 'Average train time - round: {}, model: {}, precision: {}, step time: {:.6f} ms, power metric: {:.6f} step/(s*Watt), power statistic: {}'.format(
        ...
        power_monitor.save_log(result_string)
    ...
    def __inference(self, precision):
        ...
        result_string = 'Average inference time - round: {}, model: {}, precision: {}, step time: {:.6f} ms, power metric: {:.6f} step/(s*Watt), power statistic: {}'.format(
                self._curr_run_index, self._name, precision, statistics.mean(step_times), 1000/statistics.mean(step_times)/power_monitor.get_average_power(), power_monitor.calculate_power_statistics()
            )
