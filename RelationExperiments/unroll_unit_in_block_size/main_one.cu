...
extern "C" cudaError_t fp_hammer(cudaStream_t s, int nblks, int unroll_num);
...
int main() {
    ...
    for(int unroll_num=1; unroll_num <= 128; unroll_num *= 2) {
        int nblks = 1024;
        ...
        // Call kernel
        fp_hammer(stream, nblks, unroll_num);
