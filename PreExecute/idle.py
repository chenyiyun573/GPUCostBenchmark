...
def generate_stats_and_plot(filename, device_count):
    # Add the results directory to the filename
    filename = os.path.join(results_dir, filename)
...
def main():
    ...
    stats_filename, plot_filename = generate_stats_and_plot(filename, device_count)
    print(f"Statistics saved to: {stats_filename}")
    print(f"Plot saved to: {plot_filename}")

    # sample once per second for 5 minutes
    filename = "device_data_per_s.csv"
    sample_and_save(filename, device_count, 1, 300)
    stats_filename, plot_filename = generate_stats_and_plot(filename, device_count)
    print(f"Statistics saved to: {stats_filename}")
    print(f"Plot saved to: {plot_filename}")
